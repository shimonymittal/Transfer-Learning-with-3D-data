...
#stratified sampling
    def split_train_test_datasets(self, points, labels, split_size):
        idx_label1 = list([(idx) for idx, label in enumerate(labels) if label == 'b'])
        idx_label2 = list([(idx) for idx, label in enumerate(labels) if label == 'g'])
        idx_label3 = list([(idx) for idx, label in enumerate(labels) if label == 'v'])
